...
from orbitclaw.cli.bootstrap_helpers import (
...
def _init_prompt_session() -> None:
    ...
    from orbitclaw.utils.helpers import get_cli_history_file
...
def onboard():
    ...
    from orbitclaw.config.loader import get_config_path, load_config, save_config
    from orbitclaw.config.schema import Config
    from orbitclaw.utils.helpers import (
        get_env_dir,
        get_env_file,
    ...
    if config_path.exists():
        ...
        if typer.confirm("Overwrite?"):
            config = Config()
            _apply_cli_recommended_tool_defaults(config)
            save_config(config)
...
def _make_single_provider(config: Config, model: str):
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        ...
        console.print("Set one in ~/.orbitclaw/config.json under providers section")
        raise typer.Exit(1)
...
def _make_provider(config: Config):
    ...
    if config.providers.endpoints:
        from orbitclaw.providers.router_provider import RouterProvider
...
def _build_agent_loop(
    *,
    ...
    Centralizes AgentLoop wiring used by gateway/agent/cron commands.
    """
...
async def _start_gateway_runtime(config: Config, *, data_dir: Path) -> _GatewayRuntimeState:
    from orbitclaw.bus.events import OutboundMessage
    from orbitclaw.bus.queue import MessageBus
    from orbitclaw.channels.manager import ChannelManager
    from orbitclaw.cron.service import CronService
    from orbitclaw.cron.types import CronJob
    from orbitclaw.heartbeat.service import HeartbeatService
    from orbitclaw.session.manager import SessionManager
    ...
    agent_task = asyncio.create_task(agent.run(), name="orbitclaw.agent.run")
    channels_task = asyncio.create_task(channels.start_all(), name="orbitclaw.channels.start_all")
...
def gateway(
    ...
    from orbitclaw.config.loader import (
        get_config_path,
        get_data_dir,
        load_config,
        load_config_strict,
    ...
    console.print(f"{__logo__} Starting orbitclaw gateway on port {port}...")
    ...
    async def run():
        ...
        try:
            ...
            _ensure_claude_code_runtime_dependencies(config, console=console)
            ...
            while True:
                ...
                try:
                    ...
                    _ensure_claude_code_runtime_dependencies(next_config, console=console)
...
def agent(
    ...
    from orbitclaw.bus.queue import MessageBus
    from orbitclaw.config.loader import get_data_dir, load_config
    from orbitclaw.cron.service import CronService
...
def _get_bridge_dir() -> Path:
    ...
    from orbitclaw.utils.helpers import get_bridge_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # orbitclaw/bridge (installed)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from orbitclaw.config.loader import get_data_dir
    from orbitclaw.cron.service import CronService
...
def cron_add(
    ...
    from orbitclaw.config.loader import get_data_dir
    from orbitclaw.cron.service import CronService
    from orbitclaw.cron.types import CronSchedule
...
def status():
    ...
    from orbitclaw.cli.diagnostics_commands import run_status
...
def doctor():
    ...
    from orbitclaw.cli.diagnostics_commands import run_doctor
...
def webui(
    ...
    from orbitclaw.webui.server import run_webui
...
def provider_login(
    ...
    from orbitclaw.providers.registry import PROVIDERS
