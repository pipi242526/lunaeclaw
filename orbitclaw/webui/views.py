...
from orbitclaw.utils.budget import (
...
from orbitclaw.webui.diagnostics import collect_config_migration_hints
from orbitclaw.webui.i18n import ui_copy as _ui_copy
from orbitclaw.webui.i18n import ui_term as _ui_term
from orbitclaw.webui.services import get_runtime_trend, record_runtime_trend_sample
from orbitclaw.webui.views_channels import render_channels as _render_channels_page
from orbitclaw.webui.views_endpoints import render_endpoints as _render_endpoints_page
from orbitclaw.webui.views_mcp import render_mcp as _render_mcp_page
from orbitclaw.webui.views_media import render_media as _render_media_page
from orbitclaw.webui.views_skills import render_skills as _render_skills_page
...
def render_dashboard(
    ...
    *,
    ...
    trend_rows = get_runtime_trend(limit=30)
    ...
    mem_trend = _sparkline([row.get("mem_used_percent") for row in trend_rows])
    load_trend = _sparkline([row.get("load1") for row in trend_rows])
    disk_trend = _sparkline([row.get("disk_used_percent") for row in trend_rows])
    ...
    for item in issues[:6]:
        if item.startswith("default model:"):
            ...
            action_rows.append(
                f"<li>{escape(item)} · <a class='mono' href='/'>{t('ensure gateway uses same ORBITCLAW_DATA_DIR and is running', '确保 gateway 使用相同 ORBITCLAW_DATA_DIR 并处于运行中')}</a></li>"
            )
        else:
            action_rows.append(
...
<div class="grid cols-2" style="margin-top:14px">
  <section class="card">
    ...
    <div class="muted" style="margin-top:8px">{t("Recent trend (latest 30 samples):", "最近趋势（最新 30 个采样）：")}</div>
    <table>
      <tr><th>{t("Memory", "内存")}</th><td><code>{mem_trend}</code></td></tr>
      <tr><th>{t("CPU load", "CPU 负载")}</th><td><code>{load_trend}</code></td></tr>
      <tr><th>{t("Disk", "磁盘")}</th><td><code>{disk_trend}</code></td></tr>
    </table>
  </section>
  <section class="card">
...
<div class="split" style="margin-top:14px">
  ...
  <section class="card">
    ...
    <div class="muted">{t("For full diagnostics, run", "更详细诊断建议使用")} <code>orbitclaw doctor</code></div>
  </section>
</div>
"""
...
def _sparkline(values: list[float | None]) -> str:
    """Render a compact, ASCII-safe trend line for dashboard telemetry."""
    ...
    hi = max(points)
    if hi <= lo:
        return ticks[len(ticks) // 2] * min(24, len(points))
    out: list[str] = []
    for v in points[-24:]:
