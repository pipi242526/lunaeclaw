...
from orbitclaw.utils.helpers import ensure_dir, get_data_path, safe_filename
...
class Session:
    ...
    def get_history(self, max_messages: int = 500) -> list[dict[str, Any]]:
        """Return unconsolidated messages in LLM format, aligned to a user turn."""
        unconsolidated = self.messages[self.last_consolidated:]
        sliced = unconsolidated[-max_messages:]

        # Drop leading non-user messages to avoid orphaned assistant/tool blocks.
...
class SessionManager:
    ...
    def _get_legacy_session_path(self, key: str) -> Path:
        """Legacy global session path (<data_dir>/sessions/)."""
        safe_key = safe_filename(key.replace(":", "_"))
