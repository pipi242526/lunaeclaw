...
from orbitclaw.agent.tools.web import WeatherTool, WebFetchTool
...
def _patch_async_client(monkeypatch: pytest.MonkeyPatch, response: httpx.Response) -> None:
    monkeypatch.setattr(
        "orbitclaw.agent.tools.web.httpx.AsyncClient",
...
async def test_weather_tool_returns_structured_error_on_fetch_failure(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr("orbitclaw.agent.tools.web.httpx.AsyncClient", lambda *args, **kwargs: _FailingAsyncClient())
    tool = WeatherTool()
