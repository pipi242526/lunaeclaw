...
class SkillsLoader:
    ...
    def _get_missing_requirements(self, skill_meta: dict) -> str:
        ...
        if requires.get("network") and os.environ.get("NANOBOT_OFFLINE") in {"1", "true", "yes"}:
            missing.append("NETWORK: disabled (NANOBOT_OFFLINE)")
        return ", ".join(missing)
    ...
    def _frontmatter_to_skill_meta(self, frontmatter: dict | None) -> dict:
        """Map simple frontmatter fields into nanobot skill metadata shape."""
        if not frontmatter:
            return {}
        skill_meta: dict = {}
